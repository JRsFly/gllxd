...
int main(int argc, char **argv) {
    ...
    if (!request.team) team = atoi(get_conf_value(conf, "TEAM"));
    if (!strlen(server_ip)) strcpy(server_ip, get_conf_value(conf, "SERVERIP"));
    if (!strlen(request.name)) strcpy(name, get_conf_value(conf, "NAME"));
    if (!strlen(request.msg)) strcpy(log_msg, get_conf_value(conf, "LOGMSG"));
    ...
    sendto(sockfd, (void *)&response, sizeof(response), 0, (struct sockaddr *)&server, len);
    if(ret < 0 || response.type == 1) {
       sendto(sockfd, (void *)&response.msg, sizeof(response.msg), 0, (struct sockaddr *)&server, len);
        exit(1);
    }
